...
def _save_h2_phase_dashboard(rows: list[dict], out_dir: str) -> str:
    ...
    scenario_palette = ['#1f3a5f', '#2e7d5b', '#c98a1f', '#b03a2e', '#6c3483']

    ncols = 2
    nrows = math.ceil(len(H2_PHASE_DASHBOARD_METRICS) / ncols)
    fig, axes = plt.subplots(
        nrows, ncols, figsize=(14, 4.5 * nrows), constrained_layout=True
    )
    ...
    for ax in axes_flat[len(H2_PHASE_DASHBOARD_METRICS):]:
        ax.set_axis_off()
